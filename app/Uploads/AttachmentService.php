...
class AttachmentService
{
     ...
     */
    public function getFileValidationRules(UploadedFile $uploadedFile): array
    {
        $fileExtension = $uploadedFile->getClientOriginalExtension();

        $generalFileLimit = config('app.attachment_upload_limit') * 1000;
        $videoFileLimit = config('app.video_upload_limit') * 1000;    

        if (in_array($fileExtension, ['pdf', 'docx', 'xlsx', 'zip'])) {
            return ['file', 'mimes:pdf,docx,xlsx,zip', 'max:' . $generalFileLimit];
        } elseif (in_array($fileExtension, ['mp4'])) {
            return ['file', 'mimes:mp4', 'max:' . $videoFileLimit];
        }

        return ['file', 'max:' . $generalFileLimit];
