...
class SearchRunner
{
    ...
    public function saveSearchHistory($searchTerm)
    {
        ...
        if ($existingHistory) {
            ...
            $searchHistory->search_term = $searchTerm;
    ...
    public function getSearchHistory(User $user, int $page = 1, int $count = 10): array
    {
        $results = collect();
        $hasMore = false;

        // สร้าง query สำหรับดึงประวัติการค้นหาของผู้ใช้
        $searchQuery = SearchHistory::where('user_id', $user->id)
            ->orderBy('count', 'desc');
        ...
        $searchResults = $searchQuery->skip(($page - 1) * $count)
            ->take($count)
            ->get();
